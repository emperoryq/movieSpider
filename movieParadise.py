# -*- coding: utf-8 -*-
"""
Created on Fri Jul 10 09:59:21 2020
#爬取一类电影的磁力链接等信息
@author: emperor
"""
...
#储存电影信息
def saveDate(data):
    file_name = '电影天堂-喜剧片.csv'#可用正则表达式，自动选取名字
    
    dataFrame = pd.DataFrame(data)
    dataFrame.to_csv(file_name, mode='a', index=False, sep=',', header=False)
#主函数
def main():
    start_url = "https://www.dy2018.com/1/index"#喜剧片开始网页，可设计处理“1”，实现不同类型的电影爬取
    depth = 1#翻页器，可以自定义翻页数
    for i in range(depth):
        print("正在爬取第" + str(1+i) + "页电影信息")
        if i == 0:
            url = "https://www.dy2018.com/1/index.html"#处理第一页,可设计处理“1”，实现不同类型的电影爬取
        else:    
            url = start_url + "_" +str(i*2) + ".html"#翻页
